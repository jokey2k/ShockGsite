...
class ReCaptchaWidget(forms.Widget):
    ...
    def get_recaptcha_widget_html(self, field_name):
        ...
        widget += u'Recaptcha.create("%s", "fdiv_%s", {theme: "clean"});' % (settings.RECAPTCHA_PUBKEY, field_name)
        widget += u'</script>'
...
class ReCaptchaField(forms.Field):
    ...
    def verify_captcha(self, challenge, response, remoteip):
        ...
        for key, value in values.iteritems():
            newvalues[key] = value.encode('utf-8')

        vrequest = urllib2.Request(url, urllib.urlencode(newvalues))
